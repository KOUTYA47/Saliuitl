...
2. **サブセット評価における分母の不整合**（2026-02-05発見）
   - effective_filesはフルデータセットに対するリスト
   - 手元のデータはサブセット（約30枚）
   - 処理数 = min(手元データ, effective_filesに含まれる数)
   ...
   **問題点**:
   - RR評価: 分母=effective画像数 → 正しい
   - LPR評価: 論文では分母=全クリーン画像数、コードでは分母=uneffective画像数
   - サブセットではeffective/uneffective比率がフルデータと異なる可能性
...
4. **Attack Detector（AD）の汎化性能**（2026-02-05追加）
   - ADはデータセット固有・攻撃パターン固有で学習されている
   - 未知のデータセット・未知の攻撃パターンへの汎化は未検証
   - 証拠: INRIA trigの検出率が54.5%と低い（学習データに含まれない形状？）
   - **懸念**: 実運用では未知の攻撃に遭遇する可能性が高い
   - **検証案**: クロスデータセット評価、クロス攻撃評価

   **関連研究（2024-2025）での改善アプローチ**:
   - NutNet: クリーン画像のみで学習（OOD検出）→ 攻撃パターン非依存
   - DIFFender: Diffusionモデルで再生成 → 学習不要
   - PATCHOUT: セマンティック一貫性で検出 → 学習不要
   - 参考: [inspire-group/adv-patch-paper-list](https://github.com/inspire-group/adv-patch-paper-list)
...
Recovery Rate（復元率）
      ↑ 制約
Level 3: インペインティング品質
  - Oracle RRとDetectedの差分で測定
  - 改善余地: 5-15%（タスク依存）
      ↑ 制約
Level 2: マスク精度
  - Biharmonic RRとOracle RRの差分で測定
  - 物体検出: 改善余地 +22.8%（高優先）
  - 画像分類: 改善余地 +0.8%（低優先）
      ↑ 制約
Level 1: 検出率 (Detection Rate) ← 最重要ボトルネック
  - 検出率が全体の上限を決定
  - 改善余地: trig攻撃で45.5%、2p攻撃で17.4%
```
...
**画像分類（CIFAR/ImageNet）**:
1. 攻撃の本質的影響の理解（上限50-65%の原因）
2. 分類器自体のロバスト化（Adversarial Training等）
3. マスク精度改善（効果薄い）

### 詳細考察ファイル

`docs/notes/oracle_test_analysis.md` を参照
